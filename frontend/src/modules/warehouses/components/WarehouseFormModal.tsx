...
export const WarehouseFormModal = ({ warehouse, onClose, onSuccess }: WarehouseFormModalProps) => {
  ...
  useEffect(() => {
    const loadBranches = async () => {
      try {
        ...
        if (!warehouse && res.data.length > 0) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: '1.5rem', overflowY: 'auto', flex: 1 }} className="custom-scrollbar">
          <form id="warehouse-form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <label style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--color-text-primary)' }}>Sucursal a la que pertenece <span style={{ color: 'var(--color-danger)' }}>*</span></label>
              <select 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <label style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--color-text-primary)' }}>Nombre del Almacén <span style={{ color: 'var(--color-danger)' }}>*</span></label>
              <input 
                type="text" 
                value={formData.name} 
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginTop: '0.5rem' }}>
              ...
              />
              <label htmlFor="warehouseActive" style={{ fontSize: '0.875rem', color: 'var(--color-text-primary)', cursor: 'pointer' }}>
                Almacén Activo
              </label>
            </div>
        ...
        <div style={{
          ...
          >
            {submitting ? 'Guardando...' : 'Guardar Almacén'}
          </button>
        </div>
      </div>
    </div>
